...
public interface TeamDao extends BaseDao<Team>{
	 ...
	 * id,项目名称
	 * 此方法用于获得当前的团信息
	 * @param valid 团的禁用和启用状态,
	 * 	可能会根据此状态进行条件查询
	 * @param projectId 项目的id,可能会
	 * 根据此id查询项目下相关团的信息
	 * @param startIndex 表示查询当前页面的
	 * 起始位置
	 * @param pageSize 表示每页显示多少条记录
	 * @return 用于封装查询结果,一般假如
	 * 结果中的数据来自于多张表,可以将每条记录
	 * 封装到一个map对象,然后再将多个map封装到list
	 * 
	 * */
	...
	/**
	 * 此方法用于实现记录的禁用和启用
	 * @param ids数组中封装的是页面上选择的记录的id
	 * @param valid封装的是禁用或启用的状态值
	 * */
	int validById(
