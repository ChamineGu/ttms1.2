...
public class ProjectServiceImpl implements ProjectService {
	...
	/**使用@Resource注解为属性注入值时,是先按类型匹配输入*/
	@Resource
	...
	public void validById(
		...
		if(valid!=0&&valid!=1)
			throw new ServiceException("valid必须是0或者1");
		//2.将字符串解析为数组
	 ...
	 */
	@Override
	public void insertObject(Project entity) {
		//对参数进行业务验证
		if(entity==null)
			throw new ServiceException("写入的数据不正确");
		//执行写入操作
	...
	public Project findObjectById(Integer id) {
		//1.判定id的有效性
		if(id==null||id<=0)
			throw new ServiceException("传递的id值无效:id="+id);
		//2.根据id查找对应的对象
	...
	public void updateObject(Project entity) {
		//1.判定参数的有效性
		if(entity==null)
			throw new ServiceException("被修改的记录不存在");
		//2.执行修改操作
